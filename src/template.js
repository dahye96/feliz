...
const Template = new Vue({
    ...
    methods: {
        ...
        switchController(arg1, arg2) {
            this.controller[arg1][arg2] = !this.controller[arg1][arg2];
        },
        getCurrentDate() {
            const ds = this.model.date.split('-');
        ...
        getDday(){
            ...
            return Math.ceil((new Date(this.model.date).getTime()
        ...
        fileUpload() {
            ...
            $imageUploader.change(function () {
                ...
                reader.onloadend = () => {
                    ...
                    swiper.appendSlide(
                        `<div class="swiper-slide" style="background-image: url(${reader.result})"></div>`,
                    );
        ...
        searchLocation() {
            naver.maps.Service.geocode({address: this.model.locationAddress}, function (status, response) {
                ...
                const myaddr = new naver.maps.Point(result.items[0].point.x, result.items[0].point.y);
                marker.setPosition(myaddr);
                map.setCenter(myaddr); // 검색된 좌표로 지도 이동
            });
        },
        updateDetailBackImg() {
            const $imageUploader = $('#backgroundUploader');
...
const swiper = new Swiper('.gallery-images.swiper-container', {
    pagination: {
